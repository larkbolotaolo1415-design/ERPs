 ...
 * Displays all documents available in the system
 */
...
$validSortColumns = ['filename', 'mime_type', 'file_size', 'uploaded_by', 'upload_date'];
...
function fetchAllDocuments(PDO $pdo, string $search = '', string $sortColumn = 'upload_date', string $sortOrder = 'DESC', int $offset = 0, int $perPage = 10): array {
    $sql = "
        SELECT id, filename, mime_type, file_size, uploaded_by, upload_date, description
        FROM documents
        WHERE 1=1
    ";
    ...
    if (!empty($search)) {
        $sql .= " AND (filename LIKE :search OR mime_type LIKE :search OR uploaded_by LIKE :search OR description LIKE :search)";
    ...
    $actualSortColumn = $sortColumnMap[$sortColumn] ?? 'upload_date';
    ...
    if (!empty($search)) {
        $countSql .= " AND (filename LIKE :search OR mime_type LIKE :search OR uploaded_by LIKE :search OR description LIKE :search)";
    }
...
try {
    $result = fetchAllDocuments($pdo, $search, $sortColumn, $sortOrder, $offset, $perPage);
} catch (Exception $e) {
...
<title>View All Documents - <?php echo htmlspecialchars($roleName); ?> Dashboard</title>
        ...
        <div class="col-12">
            <div class="welcome-card">
                <div class="card-body p-4">
                    ...
                    <p class="mb-0">View and manage all documents available in the system.</p>
                ...
                <div class="card-body">
                    ...
                    <div class="search-box">
                        <form method="GET" action="" class="d-flex gap-2">
                            ...
                            <?php if (!empty($search)): ?>
                                <a href="view_all_documents.php" class="btn btn-secondary">
                    ...
                    <?php if (empty($documents)): ?>
                        <div class="text-center text-muted py-5">
                            ...
                            <?php else: ?>
                                <p class="small">No documents have been uploaded yet.</p>
                            <?php endif; ?>
                        </div>
                    <?php else: ?>
                        <div class="fixed-table">
                            <table class="table table-sm align-middle">
                                <thead>
                                    <tr>
                                        <th><i class="fas fa-file me-2"></i></th>
                                        <th onclick="window.location.href='<?php echo getSortUrl('filename', $sortColumn, $sortOrder); ?>'">
                                            Document Name <?php echo getSortIcon('filename', $sortColumn, $sortOrder); ?>
                                        </th>
                                        <th onclick="window.location.href='<?php echo getSortUrl('mime_type', $sortColumn, $sortOrder); ?>'">
                                            Type/Category <?php echo getSortIcon('mime_type', $sortColumn, $sortOrder); ?>
                                        </th>
                                        <th onclick="window.location.href='<?php echo getSortUrl('file_size', $sortColumn, $sortOrder); ?>'">
                                            File Size <?php echo getSortIcon('file_size', $sortColumn, $sortOrder); ?>
                                        </th>
                                        <th onclick="window.location.href='<?php echo getSortUrl('uploaded_by', $sortColumn, $sortOrder); ?>'">
                                            Uploaded By <?php echo getSortIcon('uploaded_by', $sortColumn, $sortOrder); ?>
                                        </th>
                                        <th onclick="window.location.href='<?php echo getSortUrl('upload_date', $sortColumn, $sortOrder); ?>'">
                                            Upload Date <?php echo getSortIcon('upload_date', $sortColumn, $sortOrder); ?>
                                        </th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><strong><?php echo htmlspecialchars($d['filename']); ?></strong></td>
                                        <td>
                                            <span class="badge bg-info">
                                                <?php echo htmlspecialchars(getFileCategory($d['mime_type'])); ?>
                                            </span>
                                        </td>
                                        <td><?php echo htmlspecialchars(formatFileSize((int)$d['file_size'])); ?></td>
                                        <td><?php echo htmlspecialchars($d['uploaded_by']); ?></td>
                                        <td>
                                            <small><?php echo htmlspecialchars(date('Y-m-d H:i', strtotime($d['upload_date']))); ?></small>
                                        </td>
                                        <td>
                                            <div class="btn-group">
                                                <a class="btn btn-sm btn-success" 
                                                   target="_blank" 
                                                   href="api/download_stream.php?id=<?php echo (int)$d['id']; ?>" 
                                                   title="View">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a class="btn btn-sm btn-primary" 
                                                   href="api/download_stream.php?id=<?php echo (int)$d['id']; ?>&download=1" 
                                                   title="Download">
                        ...
                        <div class="mt-3 text-muted small">
                            Showing <?php echo count($documents); ?> of <?php echo $totalCount; ?> document(s)
                        </div>
                    <?php endif; ?>
                </div>
            </div>
